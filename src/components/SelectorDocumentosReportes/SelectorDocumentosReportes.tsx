...
import style from './SelectorDocumentosReportes.module.css';

interface File {
    id_reporte: number;
    registro_participantes: string | null;
    evaluacion_desempeno: string | null;
}

const SelectorDocumentosReportes: React.FC = () => {
    ...
    const handleDelete = async (fileId: number) => {
        ...
        try {
            const response = await fetch(`https://drftallerotecdj.onrender.com/talleres/api/reportes/${fileId}/`, {
                method: 'DELETE',
            });
        ...
        <div className={`${style['container']}`}>
            ...
            {files.length === 0 ? (
                ...
                <ul className={`${style['file-list']}`}>
                    {files.map((file) => (
                        <li key={file.id_reporte} className={`${style['file-item']}`}>
                            <div className={`${style['file-info']}`}>
                                {file.registro_participantes && (
                                    <button
                                        onClick={() => downloadFile(file.registro_participantes)}
                                        className={`${style['file-link']}`}
                                    >
                                        {getFileNameFromUrl(file.registro_participantes)}
                                    </button>
                                )}
                            ...
                            <button
                                onClick={() => handleDelete(file.id_reporte)}
                                disabled={isDeleting === file.id_reporte}
                                className={`${style['delete-button']}`}
                            >
                                {isDeleting === file.id_reporte ? 'Eliminando...' : 'Eliminar'}
                            </button>
                        </li>
                    ))}
                </ul>
...
export default SelectorDocumentosReportes;
